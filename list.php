...
class PDFMaker_SavePDFTemplate_Action extends Vtiger_Action_Controller {
    ...
    //Al editar un formato, y seleccionar entre activar o desactivar el formato, esto queda para todo los usuarios 
    function _pdfmaker_userstatus($templateid, $is_active, $user_id){
        $adb = PearDatabase::getInstance();
        $sql = "SELECT vtiger_users.id FROM vtiger_users WHERE not vtiger_users.id in (SELECT vtiger_pdfmaker_userstatus.userid FROM vtiger_pdfmaker_userstatus where templateid = {$templateid} GROUP BY vtiger_pdfmaker_userstatus.userid)";
        ...
        if($nrown > 0){
            for ($i = 0; $i < $nrown; $i++) {
                ...
                $id = $result['id'];
                $if_it_exists = $adb->query("SELECT * FROM vtiger_pdfmaker_userstatus WHERE userid = {$id} and templateid = {$templateid}");
                $if_it_exists_userpdfmaker = (int) $adb->num_rows($if_it_exists);
                if($if_it_exists_userpdfmaker == 0){
                    $sql = "INSERT INTO vtiger_pdfmaker_userstatus(templateid, userid, is_active, is_default, sequence) VALUES({$templateid}, {$id}, {$is_active}, 0, 1) ";
                    $disabled = $adb->query($sql);
                }
            }
        } else if (!empty($templateid)) {
            $adb->query("UPDATE vtiger_pdfmaker_userstatus SET is_active = {$is_active} WHERE templateid = {$templateid}");
        }
    }
...
//SHARING
        ...
        if ($S_Data['action'] == 'SavePDFTemplate') {
            $this->_pdfmaker_userstatus($templateid, $is_active, $cu_model->id);
        }
