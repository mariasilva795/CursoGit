...
class PDFMaker_SavePDFTemplate_Action extends Vtiger_Action_Controller {
    ...
    // function _pdfmaker_userstatus($idtemplate, $status_num){
    //     $adb = PearDatabase::getInstance();
    //     $userid = $adb->query("SELECT id FROM vtiger_users;");
    ...
    //             $if_it_exists = $adb->query("SELECT * FROM vtiger_pdfmaker_userstatus WHERE userid = {$id} and templateid = {$idtemplate}");
    //             $if_it_exists_userpdfmaker = (int) $adb->num_rows($if_it_exists);
            
    //             if($if_it_exists_userpdfmaker == 0){
    //                 $sql = "INSERT INTO vtiger_pdfmaker_userstatus(templateid, userid, is_active, is_default, sequence) VALUES({$idtemplate}, {$id}, {$status_num}, 1 ) ";
    //                 $disabled = $adb->query($sql);
    //             }else{
    //                 $update = $adb->query("UPDATE vtiger_pdfmaker_userstatus SET is_active = {$status_num} WHERE templateid = {$idtemplate} AND userid = {$id}");
    //             }
    //         }
    //     }
    // }
...
//      in case of both is_default_* are checked
        ...
        //$this->_pdfmaker_userstatus($templateid, $is_active);
//SHARING
